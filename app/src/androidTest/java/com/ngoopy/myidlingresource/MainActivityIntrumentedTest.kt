...
class MainActivityIntrumentedTest {
    ...
    @Test
    fun checkText() {
        onView(withId(R.id.tv)).check(matches(withText(instrumentalContext.getString(R.string.prepare))))
        onView(withText(instrumentalContext.getString(R.string.start))).perform(click())
        onView(withId(R.id.tv)).check(matches(withText(instrumentalContext.getString(R.string.delay2))))
    }
}